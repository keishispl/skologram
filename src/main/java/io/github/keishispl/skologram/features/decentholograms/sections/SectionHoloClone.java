package io.github.keishispl.skologram.features.decentholograms.sections;
...
@Name("DecentHolograms - Clone")
@Examples({
        "clone decenthologram named \"cat\":",
        "\tname: \"dog\"",
        "\tlocation: player's location"})
@Description("Clones a decenthologram")
@Since("1.0")
@RequiredPlugins("DecentHolograms")

public class SectionHoloClone extends Section {

    private static final EntryValidator.EntryValidatorBuilder ENTRY_VALIDATOR = EntryValidator.builder();

    static {
        if (Bukkit.getServer().getPluginManager().isPluginEnabled("DecentHolograms")){
            Skript.registerSection(SectionHoloClone.class,
                    "[temp:temp[orar[il]y]] clone [decenthologram[s]] %decenthologram%"
            );
